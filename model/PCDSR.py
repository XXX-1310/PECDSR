...
class PCDSR(torch.nn.Module):
    def __init__(self, opt):
        ...
        #                                     padding_idx=self.opt["itemnum"] - 1)
        self.item_emb = self.load_plm_embedding()#64094*768
        self.prompt_emb = torch.nn.Parameter(torch.randn(2, self.item_emb.embedding_dim))
        self.prompt_emb.data.copy_(self.item_emb.weight[-3:-2,:])
        self.trans_dim_1 = torch.nn.Linear(self.item_emb.embedding_dim,self.opt["hidden_units"])
        self.mlp = NonLinearMLP(self.item_emb.embedding_dim,256)

        self.prompt_trans_A = torch.nn.Linear(self.item_emb.embedding_dim,self.opt["hidden_units"])
        self.prompt_trans_B = torch.nn.Linear(self.item_emb.embedding_dim,self.opt["hidden_units"])

        self.D_X = Discriminator(self.item_emb.embedding_dim, self.item_emb.embedding_dim)
    ...
    def load_plm_embedding(self):
        emb_info_1 = './dataset/Food-Kitchen/emb_info.feat'
        loaded_feat = np.fromfile(emb_info_1,dtype=np.float32).reshape(-1, 768)
        loaded_feat = np.vstack([loaded_feat,loaded_feat[-1]])
        plm_embedding = torch.nn.Embedding(self.opt["itemnum"]+2, 768, padding_idx=0)
    ...
    def forward(self, o_seqs, x_seqs, y_seqs, position, x_position, y_position,time_matrix,time_matrix_x,time_matrix_y,seqs):
        seqs = self.trans_dim_1(seqs)#256，15，256
