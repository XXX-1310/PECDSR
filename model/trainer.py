...
class CDSRTrainer(Trainer):
    ...
    def train_batch(self, batch):
        ...
        # #截断SVD
        k = self.opt['k']
        seqs = self.model.item_emb(seq)
        A_prompt = self.model.prompt_emb[0]
        B_prompt = self.model.prompt_emb[1]
        A_prompt_fea = self.model.prompt_trans_A(A_prompt)
        B_prompt_fea = self.model.prompt_trans_B(B_prompt)
        U, S, V = torch.svd_lowrank(seqs,q=k)
        seqs = torch.matmul(torch.matmul(U, torch.diag_embed(S)), V.transpose(1,2))#torch.Size([256, 15, 768])
        #seqs = self.model.mlp(seqs)

        seqs_fea, x_seqs_fea, y_seqs_fea = self.model(seq,x_seq, y_seq, position, x_position, y_position,time_matrix,time_matrix_x,time_matrix_y,seqs)
        ...
        #x_nonzero_mean = torch.true_divide(re_x_weight.sum(dim=1), (re_x_weight != 0).sum(dim=1).float())
        re_x_mask = re_x_weight > 0
        re_y_weight = y_seqs_fea.matmul(torch.transpose(x_seqs, 1, 2)).squeeze(-1)
        #y_nonzero_mean = torch.true_divide(re_y_weight.sum(dim=1), (re_y_weight != 0).sum(dim=1).float())
        re_y_mask = re_y_weight > 0
        ...
        share_x_result =  self.model.lin_X(seqs_fea[:,-used:] + A_prompt_fea) # b * seq * X_num
        share_y_result = self.model.lin_Y(seqs_fea[:, -used:] + B_prompt_fea)  # b * seq * Y_num
    ...
    def test_batch(self, batch):
        ...
        for id, fea in enumerate(seqs_fea): # b * s * f
            if XorY[id] == 0:
                ...
                X_score = self.model.lin_X(share_fea + specific_fea + A_prompt_fea).squeeze(0)
                ...
                Y_score = self.model.lin_Y(share_fea + specific_fea + B_prompt_fea).squeeze(0)
